...
// Only MSCS Hawaii (owner_id=1) or admins can archive
...
$show = $_GET['show'] ?? $_POST['show'] ?? 'active';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $pdo->prepare("UPDATE vessels SET is_active = 0, archived_at = NOW() WHERE vessel_id = ?");
    $stmt->execute([$vessel_id]);

    header('Location: manage_vessels.php?status=' . urlencode($show) . '&archive=success');
    exit;
}
    ...
    <div class="app-page">
        <div class="app-container pb-5">
            ...
            <div class="card shadow-sm page-header-card mb-3">
                <div class="card-body">
                    <h1 class="h3 mb-1">Archive Vessel</h1>
                    <p class="page-meta"><?= h($vessel['vesselName']) ?></p>
                </div>
            </div>

            <form id="archiveVesselForm" method="post">
                ...
                <div class="card shadow-sm action-card mb-4">
                    <div class="card-body">
                        <div class="alert alert-warning mb-0">
                            This will archive <strong><?= h($vessel['vesselName']) ?></strong> and remove it from active operations until restored.
                        </div>
                    </div>
                </div>
                ...
                <div class="d-none d-lg-flex justify-content-between gap-2">
                    ...
                    <button type="submit" class="btn btn-warning">Archive Vessel</button>
                </div>
            </form>
...
<div class="sticky-mobile-actions d-lg-none py-2 px-3">
    <div class="app-container px-0">
        <div class="d-grid gap-2">
            <button type="submit" form="archiveVesselForm" class="btn btn-warning">Archive Vessel</button>
            <a href="manage_vessels.php" class="btn btn-outline-secondary">Cancel</a>
        </div>
    </div>
</div>
